<?php
/*+**********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 * 
 * Greek Translation Project initiated : 13 February 2014
 * Repository : https://github.com/cerebrux/vtiger6-greek-translation
 * Author(s):
 *  -- Salih Emin (http://www.linkedin.com/in/salihemin)
 *  -- Mathias Stavrou (http://mstavrou.com/)
 *  -- Konstantinos Paraskeuopoulos (https://www.linkedin.com/in/kparaskevopoulos)
 *  -- Dimitris Anagnostou
 *  -- More: https://github.com/cerebrux/vtiger6-greek-translation/graphs/contributors
 *************************************************************************************/
$languageStrings = array(
	'action' => 'ενέργεια',
	'ADD_RULES' => 'Add Rules', 
	'After Scan' => 'Μετά την σάρωση',
	'ALL' => 'Όλα',
	'Begins With' => 'αρχίζει',
	'body' => 'σώμα',
	'Connect URL' => 'Σύνδεση Url',
	'Contains' => 'Περιέχει',
	'CREATE_Accounts_SUBJECT' => 'Create Organization',
	'CREATE_Contacts_SUBJECT' => 'Create Contact',
	'CREATE_HelpDesk_FROM' => 'Δημιουργία Δελτίου Τ.Υ',
	'CREATE_Leads_SUBJECT' => 'Create Lead',
	'Do Not Validate SSL Certificate'=> 'Μην Επικυρώσετε το SSL Πιστοποιητικό',
	'Ends With' => 'τελειώνει',
	'Equals' => 'ίσο ',
	'fromaddress' => 'από',
	'LBL_ACTION' => 'Ενέργεια',
	'LBL_ACTIONS' => 'Actions',
	'LBL_ADDING_NEW_MAILBOX' => 'Προσθήκη νέου Γραμμτοκιβωτίου',
	'LBL_ADD_RECORD' => 'Προσθήκη Γραμματοκιβώτιο',
	'LBL_ADD_RULE' => 'Προσθήκη Κανόνα',
	'LBL_ALL_CONDITIONS' => 'Όλες οι προϋποθέσεις',
	'LBL_ALL_MESSAGES_FROM_LAST_SCAN' => 'Όλα τα μηνύματα από την τελευταία σάρωση',
	'LBL_ANY_CONDITIOn' => 'κάθε Κατάσταση',
	'LBL_BACK_TO_MAILBOXES' => 'Πίσω στο Γραμματοκιβώτιο',
	'LBL_BODY' => 'Σώμα',
	'LBL_CONDITIONS' => 'Συνθήκες', 
	'LBL_CONNECTION_ERROR' => 'Η σύνδεση με το γραμματοκιβώτιο απέτυχε. Ελέγξτε την δικτυακή συνδεσή σας και δοκιμάστε ξανά.', 
	'LBL_CONNECTION_TO_MAILBOX_FAILED' => 'Η Σύνδεση με το γραμματοκιβώτιο απέτυχε!<br>Οι ειδικοί χαρακτήρες δεν επιτρέπονται για όνομα_διακομιστή και το όνομα χρήστη.',
	'LBL_CONVERT_EMAILS_TO_RESPECTIVE_RECORDS' => 'Μετατρέψτε τα email στις αντίστοιχες εγγραφές τους',
	'LBL_CREATE_MAILBOX' => 'Δημιουργία Γραμματοκιβωτίου',
	'LBL_CREATE_MAILBOX_NOW' => 'Δημιουργία Γραμματοκ. τώρα',
	'LBL_DELETED_SUCCESSFULLY' => 'Διαγράφηκε με Επιτυχία', //Duplicated key-value pair
	'LBL_DELETE_RULE' => 'Διαγραφή Κανόνα',
	'LBL_DISABLE' => 'Ανενεργό',
	'LBL_DO_NOT_VALIDATE_SSL_CERTIFICATE' => 'Μην Επικυρώσετε το SSL Πιστοποιητικό',
	'LBL_DRAG_AND_DROP_BLOCK_TO_PRIORITISE_THE_RULE' => 'Ο αριθμός κανόνας υποδηλώνει την προτεραιότητα. Σύρετε για να αλλάξετε προτεραιότητα',
	'LBL_EDIT_MAILBOX' => 'Επεξεργασία Γραμματοκιβωτίου',
	'LBL_ENABLE' => 'Ενεργό',
	'LBL_FINISH' => 'Τέλος',
	'LBL_FOLDERS_INFO_IS_EMPTY' => 'Οι Πληροφορίες φακέλου είναι άδειες',
	'LBL_FOLDERS_SCANNED' => 'Σαρώθηκαν οι φάκελοι',
	'LBL_FROM' => 'Από',
	'LBL_I_DONT_KNOW' => 'Δεν Γνωρίζω',
	'LBL_IS_IN_RUNNING_STATE' => 'βρίσκεται σε κατάσταση λειτουργίας',
	'LBL_MAILBOX' => 'Γραμματοκυβότιο', 
	'LBL_MAILCONVERTER_DESCRIPTION' => "Ο μετατροπαία Email σας επιτρέπει να ρυθμίσετε το γραμματοκιβώτιό σας ωστε να σαρώσει τα μηνύματα σας και να δημιουργήσει τις κατάλληλες εγγραφές στο vtiger CRM.<br />Θα πρέπει επίσης να καθοριστούν κανόνες για να ορίσετε τις ενέργειες που πρέπει να εκτελεστούν στο email σας <br. />Τα Email σας σαρώνονται αυτόματα, εκτός και αν έχετε απενεργοποιήσει αυτή την δυνατότητα στο Χρονοδιάγραμμα Εργασιών του συστήματος.<br /><br /><br />",
	'LBL_MARK_MESSAGE_AS' => 'Επισήμανση μηνύματος ως',
	'LBL_MARK_MESSAGES_AS_READ' => 'Σήμανση μηνυμάτων ως αναγνωσμένα',
	'LBL_MARK_READ' => 'Μαρκαρισμένα ως Διαβασμένα',
	'LBL_MATCH' => 'Ταιριάζει',
	'LBL_MAX_LIMIT_ONLY_TWO' => 'Μπορείτε να ρυθμίσετε μόνο δύο γραμματοκιβώτια',
	'LBL_NEXT' => 'Επόμενο',
	'LBL_PRIORITY' => 'Προτεραιότητα', //Duplicated key-value pair
	'LBL_RULE_DELETION_FAILED' => 'Η διαγραφή του κανόνα απέτυχε', //Duplicated key-value pair
	'LBL_RULE' => 'Κανόνας',
	'LBL_RULES_LIST' => 'Λίστα Κανόνων',
	'LBL_RULES_SEQUENCE_INFO_IS_EMPTY' => 'Οι Πληροφορίες Αλληλουχίας των κανόνων είναι άδειες',
	'LBL_SAVED_SUCCESSFULLY' => 'αποθηκεύτηκε με επιτυχία',
	'LBL_SCANED_SUCCESSFULLY' => 'Σαρώθηκε με επιτυχία',
	'LBL_SCANΝED_SUCCESSFULLY' => 'Σαρώθηκε με επιτυχία',
	'LBL_SCAN_NOW' => 'Σάρωση τώρα',
	'LBL_SELECT_FOLDERS' => 'Επιλογή Φακέλου',
	'LBL_SEQUENCE_UPDATED_SUCCESSFULLY' => 'Η Ακολουθία ενημερώθηκε με επιτυχία',
	'LBL_STATUS_MESSAGE' => 'Ελέγξτε για να γίνει ενεργό',
	'LBL_UNREAD_MESSAGES_FROM_LAST_SCAN' => 'Μη αναγνωσμένων μηνυμάτων από την τελευταία σάρωση',
	'LBL_UNSELECT_ALL' => 'Αποεπιλογή Όλων',
	'LBL_UPDATE_FOLDERS' => 'Ενημέρωση Φακέλλων',
	'LBL_VALIDATE_SSL_CERTIFICATE' => 'Επικύρωση SSL Πιστοποιητικού',
	'LINK_Accounts_FROM' => 'Προσθήκη στο Πελάτη [ΑΠΟ]',
	'LINK_Accounts_TO' => 'Προσθήκη στο Πελάτη [TO]',
	'LINK_Contacts_FROM' => 'Προσθήκη στην Επαφή [ΑΠΟ]',
	'LINK_Contacts_TO' => 'Προσθήκη στην Επαφή [TO]',
	'LINK_Leads_FROM' => 'Προσθήκη στην προοπτική [ΑΠΟ]',
	'LINK_Leads_TO' => 'Προσθήκη στην προοπτική [ΠΡΟΣ]', 
	'Look For' => 'Δες για' 
	'MAILBOX_DETAILS' => 'Λεπτομέριες Γραμματοκιβωτίου',
	'MAILBOX' => 'Γραμματοκιβώτιο',
	'MailConverter_Description' => 'Μετατρέψει τα ηλεκτρονικά ταχυδρομεία με τις αντίστοιχες εγγραφές',
	'MailConverter' => 'Μετατροπέας Ηλεκτρονικού Ταχυδρομείου',
	'markas' => 'Μετά την σάρωση',
	'matchusing' => 'ταιριάζει',
	'Not Contains' => 'δεν Περιέχει',
	'Not Equals' => 'δεν είναι ίσο',
	'Password' => 'Κωδικός',
	'PRIORITISE_MESSAGE' => 'Σύρετε το μπλοκ για να δοθεί προτεραιότητα ',
	'Protocol' => 'Πρωτόκολλο',
	'Regex' => 'Regex',
	'RULE' => 'Κανόνας',
	'Scanner Name' => 'Όνομα σαρωτή',
	'SEEN' => 'Διαβασμένα',
	'SELECT_FOLDERS' => 'Επιλογή φακέλων', 
	'Server' => 'Όνομα διακομιστή',
	'SSL Method' => 'Μέθοδος SSL', 
	'SSL Type' => 'Τύπος SSL', 
	'Status' => 'Κατάσταση',
	'subject' => 'αντικείμενο',
	'Time Zone' => 'Ζώνη ώρας', 
	'toaddress' => 'Προς',
	'TO_CHANGE_THE_FOLDER_SELECTION_DESELECT_ANY_OF_THE_SELECTED_FOLDERS' => 'Για να αλλάξετε την επιλογή φακέλων, αποεπιλέξτε κάποιο απο τους επιλεγμένους φακέλους', 
	'UNSEEN' => 'Αδιάβαστα',
	'UPDATE_HelpDesk_SUBJECT' => 'Ενημέρωση Δελτίου Τ.Υ',
	'User Name' => 'Όνομα Χρήστη' , 
	'Validate SSL Certificate' => 'Επικύρωση πιστοποιητικού SSL',
);

$jsLanguageStrings = array(
    'JS_MAILBOX_DELETED_SUCCESSFULLY' => 'Το Γραμματοκιβώτιο διαγράφηκε με επιτυχία',
	'JS_MAILBOX_LOADED_SUCCESSFULLY' => 'Το Γραμματοκιβώτιο Φορτώθηκε με επιτυχία',
);	
