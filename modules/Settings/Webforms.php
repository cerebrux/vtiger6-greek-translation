<?php
/*+***********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 * 
 * Greek Translation Project initiated : 13 February 2014
 * Repository : https://github.com/cerebrux/vtiger6-greek-translation
 * Author(s):
 *  -- Salih Emin (http://www.linkedin.com/in/salihemin)
 *  -- Mathias Stavrou (http://mstavrou.com/)
 *  -- Konstantinos Paraskeuopoulos (https://www.linkedin.com/in/kparaskevopoulos)
 *  -- Dimitris Anagnostou
 *  -- More: https://github.com/cerebrux/vtiger6-greek-translation/graphs/contributors
 *************************************************************************************/
$languageStrings = array(
	'Captcha Enabled' => 'Captcha Ενεργό',
	'Enabled' => 'Κατάσταση',
	'LBL_ADD_FIELDS' => 'Προσθήκη πεδίων',
	'LBL_ALLOWS_YOU_TO_MANAGE_WEBFORMS' => 'Σας επιτρέπει να διαχειρίζεστε τις Δικτυακές Φόρμες',
	'LBL_ASSIGN_ROUND_ROBIN' => 'Ανάθεση χρηστών σε Round Robin',
	'LBL_ASSIGN_USERS' => 'Εκχώρηση χρήστη', 
	'LBL_DUPLICATES_EXIST' => 'Το όνομα της Δικτυακής Φόρμας υπάρχει ήδη',
	'LBL_EMBED_THE_FOLLOWING_FORM_IN_YOUR_WEBSITE' => 'Ενσωματώστε την παρακάτω φόρμα στην ιστοσελίδα σας',
	'LBL_ENABLE_TARGET_MODULES_FOR_WEBFORM' => 'Ενεργοποίηση αρθρωμάτων για την δικτυακή φόρμα', 
	'LBL_FIELD_INFORMATION' => 'Πληροφορίες Πεδίου',
	'LBL_FIELD_NAME' => 'Όνομα Πεδίου',
	'LBL_HIDDEN' => 'Κρυφό',
	'LBL_LABEL' => 'ετικέτα',
	'LBL_MANDATORY' => 'Υποχρεωτικό',
	'LBL_OVERRIDE_VALUE' => 'Αντικατάσταση Τιμής',
	'LBL_ROUNDROBIN_USERS_LIST' => 'Λίστα χρηστών Round Robin',
	'LBL_SAVE_FIELDS_ORDER' => 'Αποθήκευση σειράς πεδίων',
	'LBL_SELECT_FIELDS_OF_TARGET_MODULE' => 'Επιλέξτε Πεδία για το Άρθρωμα προορισμού...',
	'LBL_SELECT_VALUE' => 'Επιλέξτε τιμή',
	'LBL_SHOW_FORM' => 'Προβολή Φόρμας', 
	'LBL_WEBFORM_INFORMATION' => 'Πληροφορίες Δικτυακής Φόρμας',
	'LBL_WEBFORM_REFERENCE_FIELD' => 'Πεδίο αναφοράς Δικτυακής Φόρμας',
	'Module' => 'Άρθρωμα',
	'Post Url' => 'Url Προηγούμενο',
	'Public Id' => 'Δημόσιο Id',
	'Return Url' => 'Url Επιστροφής',
	'SINGLE_Settings:Webforms' => 'Δικτυακή Φόρμα',
	'SINGLE_Webforms' => 'Δικτυακή Φόρμα',
	'WebForm Name' => 'Όνομα Δικτυακής Φόρμα',
);

$jsLanguageStrings = array(
	'JS_LABEL' => 'ετικέτα',
	'JS_LOADING_TARGET_MODULE_FIELDS' => 'Φόρτωση Πεδίων του Αρθρώματος',
	'JS_MANDATORY_FIELDS_WITHOUT_OVERRIDE_VALUE_CANT_BE_HIDDEN' => 'Υποχρεωτικά πεδία χωρίς τιμές παράκαμψης δεν μπορούν να είναι κρυφά',
	'JS_REFERENCE_FIELDS_CANT_BE_MANDATORY_WITHOUT_OVERRIDE_VALUE' => 'Πεδία αναφοράς δεν μπορούν να είναι υποχρεωτικά χωρίς τιμές παράκαμψης',
	'JS_SELECT_AN_OPTION' => 'Επιλέξτε μια επιλογή',
	'JS_TYPE_TO_SEARCH' => 'Πληκτρολογίστε για αναζήτηση',
	'JS_WEBFORM_DELETED_SUCCESSFULLY' => 'Η Δικτυακή Φόρμα διαγράφτηκε επιτυχώς',
	"JS_WEBFORM_WITH_THIS_NAME_ALREADY_EXISTS" => 'Η δικτυακή φόρμα υπάρχει ήδη με αυτό το όνομα',
);
